...
- メッセージ内のテキストを選択して `Ctrl+Shift+C` を押しても、選択範囲だけをコピーできます。選択がない場合は、マウスを重ねているかフォーカス中のメッセージが対象です。`Shift+Command+C` は Chrome の予約ショートカットなので使用しません。
...
`clipboardWrite` は、ユーザーがMDボタンまたはショートカットを操作したときにMarkdownをクリップボードへ書き込むために使います。`content_scripts.matches` で `https://app.slack.com/*` にだけ拡張を読み込み、追加のホスト権限は要求しません。
